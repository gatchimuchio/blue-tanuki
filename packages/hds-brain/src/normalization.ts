...
const CONTROL_NAMES = new Map<number, { kind: DetectedControlChar["kind"]; name: string }>([
...
export function normalizeForDetection(raw: string): DetectionNormalization {
...
function detectControlChars(raw: string): DetectedControlChar[] {
  const found: DetectedControlChar[] = [];
  let index = 0;
  for (const ch of raw) {
