 ...
 */
export async function readManifest(manifest_path) {
    let raw;
    try {
        ...
        const err = e;
        throw new Error(`readManifest: cannot read ${manifest_path}: ${err.code ?? err.message}`);
    }
    let parsed;
    try {
        ...
        throw new Error(`readManifest: invalid JSON at ${manifest_path}: ${e.message}`);
    }
 ...
 */
export function manifestPathFor(package_dir) {
