...
export function resolveProcess(req, actor) {
    const meta = req.metadata ?? {};
    const trustedAuthorityContext = isTrustedAuthorityContext(meta);
    const explicitKind = trustedAuthorityContext
...
function inferActorKind(req) {
    const meta = req.metadata ?? {};
    const trigger = isTrustedAuthorityContext(meta) ? stringField(meta, "trigger_kind") : undefined;
    if (trigger === "cron" || req.channel === "cron")
        return "cron";
    if (trigger === "webhook" || req.channel === "webhook")
        return "webhook";
    if (req.channel === "system")
        return "system";
    if (req.channel === "cli" && req.user === "local-user")
        return "owner";
    return "user";
}
function inferTrustLevel(req, kind) {
    if (kind === "owner")
        return "owner";
...
function inferProcessKind(req, actor) {
    const trimmed = req.content.trim();
    if (actor.actor_kind === "cron")
        return "cron";
    if (actor.actor_kind === "webhook")
        return "webhook";
    if (trimmed.startsWith("tool:") || trimmed.startsWith("/tool") || req.metadata?.["blue_tanuki.tool_call"] || req.metadata?.tool_call)
        return "tool";
    if (isTrustedAuthorityContext(req.metadata ?? {}) && (req.metadata?.["blue_tanuki.resume"] || req.metadata?.resume))
        return "approval";
    return "chat";
}
function triggerKind(process_kind) {
    if (process_kind === "cron")
        return "cron";
    if (process_kind === "webhook")
        return "webhook";
    if (process_kind === "approval")
        return "resume";
    return "inbound";
}
function allowedActorKinds(process_kind) {
    if (process_kind === "approval")
        return ["owner", "user"];
    if (process_kind === "cron")
        return ["cron", "system", "owner"];
    if (process_kind === "webhook")
        return ["webhook"];
    return ["owner", "user", "system", "cron", "webhook"];
}
function executionPolicy(process_kind) {
 ...
 */
function isTrustedAuthorityContext(meta) {
