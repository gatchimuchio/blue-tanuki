...
import { canonicalizeMemoryEntry, decodeMemoryEntry, encodeMemoryEntry, } from "./codec.js";
...
export class LongTermMemoryStore {
    ...
    findByProcess(process_id, limit = 20) {
    ...
    loadFromFile() {
        ...
        this.entries = lines.map((line) => decodeMemoryEntry(line));
        if (!this.verify()) {
            throw new Error(`LongTermMemoryStore: chain verification failed on load from ${this.filepath}`);
        }
    }
}
function entryContains(entry, needle) {
    const hay = [
        entry.request_id,
        entry.goal,
...
function memoryTags(log) {
    ...
    for (const value of log.frame.world_closure.x)
