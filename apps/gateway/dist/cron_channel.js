...
export function dailyBriefCronFromEnv(env = process.env) {
    ...
    if (!target)
        throw new Error("BLUE_TANUKI_DAILY_BRIEF_TARGET is required when daily brief is enabled");
    return {
        enabled,
        channel,
        target,
...
export function delayUntilLocalTime(hhmm, now) {
    const m = /^(\d{1,2}):(\d{2})$/.exec(hhmm);
    if (!m)
        throw new Error(`invalid daily brief time: ${hhmm}`);
...
//# sourceMappingURL=cron_channel.js.map