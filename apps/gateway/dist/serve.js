...
import { Executor, ToolRegistry, createLogger, } from "@blue-tanuki/core";
...
import { InboundRouter, OutboundDispatcher, } from "@blue-tanuki/channel-base";
...
export async function serve() {
    ...
    plugins.requirePermissions("@blue-tanuki/channel-webchat", webchatPermissions, "read/register webchat channel");
     ...
     */
    async function executeAndEcho(cmd, log, origin, opts = {}) {
        if (!opts.skip_approval) {
            ...
            if (evaluation.decision === "ask") {
                ...
                await dispatcher.dispatch({ channel: origin.channel, target: replyTarget(origin), content: approvalRequiredMessage(evaluation, cmd.id, issued?.token) }, { command_id: `approval-${cmd.id}`, upstream_commit_hash: log.commit.hash });
            ...
            if (evaluation.decision === "deny") {
                ...
                if (content)
                    await dispatcher.dispatch({ channel: origin.channel, target: replyTarget(origin), content }, { command_id: cmd.id, upstream_commit_hash: log.commit.hash });
    ...
    async function resumePendingApproval(command_id, verdict, ctx) {
    ...
    webchat = plugins.createChannel({
        ...
        constructor_args: [{
                ...
                onResume: async (request_id, verdict, ctx) => {
    ...
    plugins.requirePermissions("@blue-tanuki/channel-telegram", telegramPermissions, "read/register telegram channel");
    const telegram = plugins.createChannel({
        ...
        constructor_args: [{
                ...
                log: (line) => telegramLog.info(line),
            }],
    });
    ...
    plugins.requirePermissions("@blue-tanuki/channel-slack", slackPermissions, "read/register slack channel");
    ...
    plugins.requirePermissions("@blue-tanuki/channel-discord", discordPermissions, "read/register discord channel");
    ...
    const handler = async (req) => {
        ...
        const issued = log.commit.decision === "SUSPEND"
            ? await webchat.issueResumeApprovalToken(req.id)
            : null;
        const human = humanizeDecision(log.commit.decision, log.commit.reason, req.id, issued?.token);
        await dispatcher.dispatch({ channel: req.channel, target: replyTarget(req), content: human }, { command_id: `notify-${req.id}`, upstream_commit_hash: log.commit.hash });
    };
...
function humanizeDecision(decision, reason, request_id, approval_token) {
    switch (decision) {
        case "SUSPEND":
            return `[suspended] Awaiting human review. reason=${reason} request_id=${request_id}${approval_token ? ` approval_token=${approval_token}` : ""}`;
        case "OUT_OF_SCOPE":
