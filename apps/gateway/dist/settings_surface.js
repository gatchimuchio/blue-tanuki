...
import { renderSetupEnvFile, setupConfigFromEnv, setupConfigToEnv, } from "./setup_config.js";
import { describeLLMCommandRoute, describeLLMConfig, } from "./llm_config.js";
...
export function settingsSurfaceAllowed(env) {
    if (!envValue(env, "BLUE_TANUKI_SETTINGS_TOKEN"))
        return false;
    if (isLoopbackHost(env.WEBCHAT_HOST))
        return true;
...
function optionalStringField(obj, key) {
    const value = obj[key];
    if (value === undefined)
        return undefined;
    if (value === null)
...
function optionalNumberField(obj, key) {
    const value = obj[key];
...
function optionalIntField(obj, key) {
...
async function readEnvFileEnv(filePath) {
    if (!filePath)
        return {};
...
function apiKeySet(env, provider) {
    if (provider === "anthropic")
        return Boolean(envValue(env, "ANTHROPIC_API_KEY"));
    if (provider === "openai") {
        return Boolean(envValue(env, "OPENAI_API_KEY", "LLM_API_KEY"));
    }
    if (provider === "openai-compatible") {
        return Boolean(envValue(env, "OPENAI_COMPAT_API_KEY", "OPENAI_API_KEY", "LLM_API_KEY"));
    }
    return false;
}
export function buildSettingsSnapshot(env, plugins) {
...
function applySettingsPatch(config, body) {
    if (isRecord(body.llm)) {
        const llm = body.llm;
        const provider = stringField(llm, "provider");
        if (provider)
            config.llm.provider = normalizeProvider(provider);
        ...
        if (maxTokens !== undefined)
            config.llm.max_tokens = maxTokens;
        const timeoutMs = optionalIntField(llm, "timeout_ms");
        if (timeoutMs !== undefined)
            config.llm.timeout_ms = timeoutMs;
    }
...
export function createWebChatSettingsSurface(opts) {
    const env = opts.env ?? process.env;
    const token = env.BLUE_TANUKI_SETTINGS_TOKEN;
    if (!settingsSurfaceAllowed(env) || !token)
        return undefined;
    return {
        token,
...
//# sourceMappingURL=settings_surface.js.map