...
import { approvalGrantFromEvaluation, buildApprovalGrant, evaluateApproval, JsonFileApprovalGrantStore, MemoryApprovalGrantStore, } from "@blue-tanuki/hds-brain";
export function buildApprovalRuntime(env = process.env) {
...
export function approvalRequiredMessage(evaluation, command_id, approval_token) {
...
export function approvalDeniedFeedback(command, reason) {
    return { command_id: command.id, status: "failed", error: `approval_denied:${reason}`, metrics: { duration_ms: 0 } };
}
function parseApprovalMode(value) { return value === "remember_this_decision" || value === "full_access" || value === "ask_every_time" ? value : "full_access"; }
function buildSystemGrants() {
    const created_at = 0;
    return [
...
//# sourceMappingURL=approval_runtime.js.map