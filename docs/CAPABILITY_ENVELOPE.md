...
## 6. Plugin Review Gate Connection

Plugin Review Gate uses this capability envelope as its deny-by-default review source.

Layer B submissions must declare every network, filesystem, process, credential, memory, and notification capability before load or execution. Declared capability is still downstream-only and still subject to HDS-BRAIN, Approval Gate, Runtime Invariants, and audit.
