...
| `SLACK_BOT_TOKEN` | Slack preview adapter | bot token | Slack app config | optional token presence | yes, Slack silent fallback | Slack delivery false / live smoke skip | rotate Slack app token, restart |
| `SLACK_APP_TOKEN` | Slack Socket Mode | app token | Slack app config | optional token presence | yes, Slack silent fallback | Socket Mode not started | rotate Slack app token, restart |
| `DISCORD_BOT_TOKEN` | Discord preview adapter | bot token | Discord developer portal | optional token presence | yes, Discord silent fallback | Discord delivery false / live smoke skip | rotate in developer portal, restart |
