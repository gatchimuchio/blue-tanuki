...
本書は BLUE-TANUKI の active internal roadmap である。外部向け比較資料、法務資料、マーケティング資料ではない。判断基準は常に safety-first の実装可能性と、HDS-BRAIN authority core を壊さないことに置く。

歴史的ロードマップは `docs/history/` に保管する。現リポジトリで確認できた旧版は v1 であり、v2 / v3 / v4 / v5 が存在しない場合は新規に捏造しない。
...
互換性は channel 数の競争ではなく、主要ユースケースを安全境界内で再現できるかで判断する。
...
重要:
BLUE-TANUKI は WhatsApp 専用の非公式実装を提供しない。
提供するのは、一般化された channel adapter interface のみである。

### Long-tail Channels

iMessage、Google Chat、Signal、Matrix、Feishu、その他 long-tail channels は first-party core ではなく、需要観測と adapter IF の成熟度を見て判断する。安全境界を破る実装ルートは採用しない。

## 4. 拡張性の定義

BLUE-TANUKI における拡張性とは、単に plugin を追加できることではない。

本リポジトリを LLM / Codex / 第三者開発者が読んだとき、
HDS-BRAIN、authority path、approval gate、runtime invariants、audit hash-chain を侵食せず、
定義済み adapter contract / capability envelope / conformance test に従って、
下流 channel / module / skill を一定品質で追加できる性質を指す。
...
拡張性の目的は「誰でも好きに拡張できること」ではなく、
「誰が、またはどのLLMが実装しても、core safety property が壊れないこと」である。
...
- capability envelope、audit trace、runtime invariant preservation を test-enforced にする
