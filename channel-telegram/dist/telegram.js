...
export class TelegramChannel {
  ...
  async send(payload, meta) {
    ...
    const body = { chat_id: payload.target, text: payload.content, disable_web_page_preview: true };
    try {
  ...
  async pollLoop(handler) {
    while (!this.stopped) {
      try {
        const r = await this.call("getUpdates", { offset: this.offset > 0 ? this.offset : undefined, timeout: this.opts.poll_timeout_sec ?? 25, allowed_updates: ["message", "edited_message"] });
  ...
  normalize(update) {
    ...
    return { id: randomUUID(), channel: "telegram", user: username || fromId, content: msg.text, timestamp: Date.now(), metadata: { reply_to: chatId, telegram_chat_id: chatId, telegram_user_id: fromId, telegram_message_id: msg.message_id, telegram_update_id: update.update_id } };
  }
  async call(method, body) {
    ...
    const res = await fetchImpl(url, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(stripUndefined(body)) });
    const text = await res.text();
    let parsed;
    try { parsed = text ? JSON.parse(text) : {}; } catch { parsed = { ok: false, description: text }; }
    if (isRecord(parsed)) return { ok: parsed.ok === true, result: parsed.result, description: typeof parsed.description === "string" ? parsed.description : res.ok ? undefined : `http_${res.status}` };
    return { ok: false, description: `http_${res.status}` };
  }
  log(line) { (this.opts.log ?? console.log)(line); }
}
function stripUndefined(value) { const out = {}; for (const [k, v] of Object.entries(value)) if (v !== undefined) out[k] = v; return out; }
function readTelegramMessageId(value) { if (!isRecord(value)) return null; const id = value.message_id; return typeof id === "number" || typeof id === "string" ? String(id) : null; }
